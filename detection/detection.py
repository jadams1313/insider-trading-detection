...
# 7. VISUALIZATION
def plot_distance_distribution(distances, threshold=None, bins=50):
    ...
    plt.title('Distribution of DTW Distances (Sample)')
...
def plot_method_comparison(results):
    ...
    plt.title('Detection Count by Method')
...
def plot_anomaly(anomaly, method_name=''):
    """Plot single DTW alignment"""
    ...
    ax.plot(signal, 'b-', linewidth=2, alpha=0.7, label='Predicted')
    ax.plot(pattern, 'r-', linewidth=2, alpha=0.7, label='Pattern')
    
    # Sample alignment lines
    step = max(1, len(path) // 15)
    for i, j in path[::step]:
        ax.plot([i, j], [signal[i], pattern[j]], 'k-', alpha=0.15, linewidth=0.5)
    
    ax.set_xlabel('Time (Days)')
    ax.set_ylabel('Normalized Volume')
    ax.set_title(f'{method_name} | Pattern {anomaly["pattern_id"]} | '
                f'Day {anomaly["actual_day"]} | DTW: {anomaly["dtw_distance"]:.3f}')
    ax.legend()
...
def plot_top_anomalies(results, method_name, top_n=5):
    """Plot top N anomalies for a method"""
    if method_name not in results or not results[method_name]:
        print(f"No anomalies for {method_name}")
        return
    
    anomalies = sorted(results[method_name], key=lambda x: x['dtw_distance'])[:top_n]
    ...
    for idx, anom in enumerate(anomalies):
        ...
        ax.set_title(f"#{idx+1}: Pattern {anom['pattern_id']}, Day {anom['actual_day']}, "
    ...
    plt.suptitle(f'{method_name} - Top {top_n} Anomalies (Lowest DTW)', 
                fontsize=13, y=1.001)
    plt.tight_layout()
    plt.show()
